...
const NewsLetterModalPopUp = () => {
  ...
  useEffect(() => {
    const hasDismissed =
      localStorage.getItem(LOCAL_STORAGE_KEY as string) === "true";
    if (!hasDismissed) {
      setOpen(true);
    }
  ...
  const handleClose = () => {
    localStorage.setItem(LOCAL_STORAGE_KEY as string, "true");
    setOpen(false);
  };
          ...
          >
            ...
            >
              <Dialog.Close asChild>
                <button
                  onClick={handleClose}
                  className="absolute top-4 right-4 text-base-black bg-white rounded-full p-4 cursor-pointer focus:outline-none"
                >
