...
interface ApiContact {
  ...
  tags?: string[] | null;
  status?: ContactRecord["status"] | null;
...
const ContactAdminPage = () => {
  ...
  const allContacts: ContactRecord[] = useMemo(() => {
    ...
    return items.map((c: ApiContact): ContactRecord => {
      ...
      return {
        ...
        channel: (c.channelIdentities?.[0]?.channel ??
          "WHATSAPP") as ContactRecord["channel"],
        status: c.status ?? (convCount > 0 ? "active" : "inactive"),
                ...
                {isLoading ? (
                  <tr>
                    <td colSpan={7} className="py-16 text-center">
                      <div className="flex flex-col items-center gap-2 text-grey-medium">
                        <Loader2 className="h-6 w-6 animate-spin" />
                        <p className="body-medium-16">Loading contacts...</p>
                      </div>
                    </td>
                  </tr>
                ) : isError ? (
