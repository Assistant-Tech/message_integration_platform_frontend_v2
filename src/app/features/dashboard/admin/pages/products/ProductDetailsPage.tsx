        ...
        >
          ...
          <div className="bg-white border border-grey-light rounded-xl p-6">
            ...
            <p className="text-grey-medium leading-relaxed text-sm">
              {product.description ||
                "This product is crafted with premium-quality materials and designed to deliver outstanding performance and durability."}
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 space-y-4 border border-grey-light">
            ...
            <div className="flex justify-between items-center borber-b border-grey-medium py-2">
              <p className="text-sm font-semibold text-grey uppercase tracking-wider">
                Variants
              </p>
              <div className="flex flex-col gap-2 text-black">
                {product.variants.map((v: any) => (
                  <div key={v.sku} className="flex items-center">
                    <h5>{v.inventory.stock}</h5>
                    {v.inventory.lowStock && (
            ...
            <div className="border-t border-grey-light pt-4">
              {product.variants[0]?.attributes.color && (
                <div className="flex justify-between items-center border-b border-grey-light pb-4">
                  ...
                  <span
                    className={`px-4 py-1 text-white rounded-full text-sm font-medium ${product.variants[0].attributes.color}`}
                  >
                    {product.variants[0].attributes.color
                      .replace("bg-", "")
                      .replace("-", " ")}
            ...
            />
            <Detail label="Status" value={product.status || "N/A"} />
          </div>
...
const Detail = ({
  ...
  <div className="flex justify-between border-b border-grey-light pb-3 py-2">
    <p className="text-sm font-semibold text-grey uppercase tracking-wider">
      {label}
    </p>
